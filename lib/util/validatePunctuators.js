  ...
  if (alignOnLongestVar) {
    ...
    if (!longestColumn) {
      items.forEach(token => context.report({
        ...
        node:    token,
        fix:     f => fixer.removeAlignmentRange(
          source,
          token,
          f,
          columnComputation.computeActualColumn(items, source)
        )
      }));
  ...
  items.forEach(punctuator => {
    ...
    if (isInvalidAlignment(cur, actual, alignOnLongestVar, alignOnLongestVar ? longestColumn : expected)) {
      context.report({
        ...
        node:    punctuator,
        fix:     fixer.fix(alignOnLongestVar ? actual : expected, cur, punctuator, source, alignOnLongestVar)
      });
    }
  });
};
