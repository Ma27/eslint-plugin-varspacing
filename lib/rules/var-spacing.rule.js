...
module.exports = function (context) {
  ...
  function declarationCheck(node) {
    function findFirstDeclaredVar(node, steps) {
      if (node.declarations.length > 1) {
        ...
        if (node.declarations[step].init === null) {
          return findFirstDeclaredVar(node, step + 1);
        }
    ...
    var first = findFirstDeclaredVar(node);
    if (typeof first.init !== 'undefined' && first.init !== null) {
      // parse through the next AST tokens in order
      // to gather the next relevant tokens being necessary
      // for the alignment validation.
      var punctuator      = source.getTokensBetween(first.id, first.init).shift(),
          nextPunctuators = findNextPunctuators(node, source, sourceLines);
